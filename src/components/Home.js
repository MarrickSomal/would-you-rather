...
function Home(props) {
  ...
  return (
    <div className={classes.root}>
      ...
      >
        <Tab label={<span className={classes.tabLabel}>Unanswered</span>} index={0} />
        <Tab label={<span className={classes.tabLabel}>Answered</span>} index={1} />
      </Tabs>
      <TabPanel label="Unanswered" value={value} index={0}>
        {userPollData.answeredPolls.map((question) => (
          <UserCard key={question.id} questionId={question.id} unansweredPolls={true} />
        ))}
      </TabPanel>
